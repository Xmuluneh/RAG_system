...
def main(num_test_output: str):
    ...
    ape_message = file_reader("../prompts/automatic-prompt-generation.txt")

    context = str(context_message)
    prompt = str(prompt_message)
    ape_prompt = str(ape_message)
    test_data = generate_test_data(prompt, context, num_test_output)
    ape = generate_prompts(ape_prompt, context, "list the main tasks of the project", "text")

    file_path = "../test-dataset/test-data.json"
    save_json(test_data, file_path)
    file_path = "../prompts-generated/prompts.json"
    save_json(ape, file_path)
